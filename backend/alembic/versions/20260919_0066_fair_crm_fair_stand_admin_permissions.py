"""Add SYSTEM-scoped Fair Stand catalog and preview admin permissions.

Revision ID: 20260919_0066
Revises: 20260903_0065
"""
...
SYSTEM_SCOPE = "system"
PERMISSIONS = (
    ("fair_crm.admin.fair_stand.catalog.read", "Read Fair Stand catalog administration"),
    ("fair_crm.admin.fair_stand.catalog.create", "Create Fair Stand catalog categories"),
    ("fair_crm.admin.fair_stand.catalog.update", "Update Fair Stand catalog categories and Item catalog fields"),
    ("fair_crm.admin.fair_stand.catalog.archive", "Archive Fair Stand catalog categories"),
                ...
                sa.text(
                    ...
                    VALUES
                        (:id, :code, 'FAIR CRM Fair Stand Admin', 'fair_crm',
                         'SYSTEM Super Admin Fair Stand catalog and preview administration', 96, TRUE,
        ...
        connection.execute(
            sa.text(
                ...
                SET name='FAIR CRM Fair Stand Admin',
                    module='fair_crm',
                    description='SYSTEM Super Admin Fair Stand catalog and preview administration',
                    sort_order=96,
                    is_system=TRUE,
...
def downgrade() -> None:
    ...
    leftover = connection.execute(
        sa.text("SELECT COUNT(*) FROM identity_permissions WHERE group_id IN (SELECT id FROM identity_permission_groups WHERE code=:code)"),
        {"code": GROUP_CODE},
    ).scalar_one()
    if leftover == 0:
        connection.execute(
