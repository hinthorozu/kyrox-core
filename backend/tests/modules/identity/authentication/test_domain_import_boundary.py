...
def _authentication_domain_root() -> Path:
    return (
...
def _collect_python_files(root: Path) -> list[Path]:
    return [path for path in root.rglob("*.py") if path.name != "__init__.py" or path.parent != root]
...
def test_authentication_domain_has_no_forbidden_imports() -> None:
    root = _authentication_domain_root()
