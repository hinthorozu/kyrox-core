...
def test_role_and_permission_repositories(db_session: Session) -> None:
    ...
    assert role_repo.get_by_id(role.id) == role
    assert role_repo.get_by_organization_and_slug(org.id, "member") == role
    assert permission_repo.get_by_code("core.user.read") == permission
    assert role_permission_repo.has_permission(role.id, permission.id) is True


def test_permission_checker_respects_membership_role(db_session: Session) -> None:
    ...
    assert checker.has_permission(user.id, org.id, "core.user.read") is True
    assert checker.has_permission(user.id, org.id, "core.user.create") is False
    assert checker.has_permission(user.id, uuid.uuid4(), "core.user.read") is False
